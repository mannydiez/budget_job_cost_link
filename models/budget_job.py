			...
			if list_of_objects:
				for rec in list_of_objects:
					for rec2 in vals['job_labour_line_ids']:
						if rec.get('group_product_id') == rec2.get('group_product_id'):
							rec.get('total_cost') += rec2.get('total_cost')
			else:
			...
			for record_acc in analytic_obj.product_budget_lines:
				if analytic_obj.product_budget_lines:
					for record_job in list_of_objects:
						log.warning('{} == {}'.format(record_job.get('group_product_id'),record_acc['group_product_id'].id))
						if record_job.get('group_product_id') == record_acc['group_product_id'].id:
							log.warning('{} > {}'.format(record_job.get('total_cost'),record_acc['planned_amount']))
							if record_job.get('total_cost') > record_acc['planned_amount']:
