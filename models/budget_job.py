			...
			if list_of_objects:
				for rec in list_of_objects:
					for rec2 in vals['job_labour_line_ids']:
						if rec[0][-1]['group_product_id'] == rec2[0][-1]['group_product_id']:
							rec[0][-1]['total_cost'] += rec2[0][-1]['total_cost']
			else:
			...
			for record_acc in analytic_obj.product_budget_lines:
				if analytic_obj.product_budget_lines:
					for record_job in list_of_objects:
						log.warning('{} == {}'.format(record_job[0][-1]['group_product_id'],record_acc['group_product_id'].id))
						if record_job[0][-1]['group_product_id'] == record_acc['group_product_id'].id:
							log.warning('{} > {}'.format(record_job[0][-1]['total_cost'],record_acc['planned_amount']))
							if record_job[0][-1]['total_cost'] > record_acc['planned_amount']:
